<?php

/* 
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
?>

    <section class="about-area pt-70">
        <div class="about-shape-2">
            <img src="<?php $vars['siteUrl']?>mod/tutor-lms/vendors/asset/login/images/about-shape-2.svg" alt="shape">
        </div>
        <div class="container">
            <div class="row">
                <div class="col-lg-6 order-lg-last">
                    <div class="about-content mt-50 wow fadeInLeftBig" data-wow-duration="1s" data-wow-delay="0.5s">
                        <div class="section-title">
                            <div class="line"></div>
                            <h3 class="title"><?php echo elgg_echo('tutor-lms:login:about:slide_1:title_1');?><span> <?php echo elgg_echo('tutor-lms:login:about:slide_1:title_2');?></span></h3>
                        </div> <!-- section title -->
                        <p class="text"><?php echo elgg_echo('tutor-lms:login:about:slide_1:description');?></p>
                        
                    </div> <!-- about content -->
                </div>
                <div class="col-lg-6 order-lg-first">
                    <div class="about-image text-center mt-50 wow fadeInRightBig" data-wow-duration="1s" data-wow-delay="0.5s">
                        <img src="<?php $vars['siteUrl']?>mod/tutor-lms/vendors/asset/login/images/about2.svg" alt="about">
                    </div> <!-- about image -->
                </div>
            </div> <!-- row -->
        </div> <!-- container -->
    </section>


    <!--====== ABOUT PART START ======-->
    
    <section class="about-area pt-70">
        <div class="container">
            <div class="row">
                <div class="col-lg-6">
                    <div class="about-content mt-50 wow fadeInLeftBig" data-wow-duration="1s" data-wow-delay="0.5s">
                        <div class="section-title">
                            <div class="line"></div>
                            <h3 class="title"><span><?php echo elgg_echo('tutor-lms:login:about:slide_2:title_1');?></span><?php echo elgg_echo('tutor-lms:login:about:slide_2:title_2');?></h3>
                        </div> <!-- section title -->
                        <p class="text"><?php echo elgg_echo('tutor-lms:login:about:slide_2:description');?></p>
                        
                    </div> <!-- about content -->
                </div>
                <div class="col-lg-6">
                    <div class="about-image text-center mt-50 wow fadeInRightBig" data-wow-duration="1s" data-wow-delay="0.5s">
                        <img src="<?php $vars['siteUrl']?>mod/tutor-lms/vendors/asset/login/images/about3.svg" alt="about">
                    </div> <!-- about image -->
                </div>
            </div> <!-- row -->
        </div> <!-- container -->
        <div class="about-shape-1">
            <img src="<?php $vars['siteUrl']?>mod/tutor-lms/vendors/asset/login/images/about-shape-1.svg" alt="shape">
        </div>
    </section>
</br>